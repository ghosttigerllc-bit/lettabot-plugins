...
interface ParsedEntity {
  type: TdlibTextEntityType['_'];
  start: number;      // Position in original string (with markdown)
  end: number;        // Position in original string (with markdown)
  content: string;    // The actual text content (without markdown syntax)
...
export function markdownToTdlib(markdown: string): TdlibFormattedText {
  ...
  // Track offset mapping: as we strip markdown syntax, we need to track
  // where each character ends up in the plain text
 ...
 * Useful when you just want to send plain text through the TDLib API.
 */
