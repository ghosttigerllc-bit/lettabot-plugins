...
import type { InboundMessage, OutboundMessage } from '../../src/core/types.js';
import type { DmPolicy } from '../../src/pairing/types.js';
import { isUserAllowed, upsertPairingRequest, approvePairingCode } from '../../src/pairing/store.js';
import { markdownToTdlib } from './format.js';
...
Reply **approve** or **deny** to this message.`;
  ...
  private async promptForInput(type: 'code' | 'password'): Promise<string> {
    ...
    if (type === 'password') {
      ...
      return this.promptHiddenInput(prompt);
    }

    // Normal input for verification codes
   ...
   * This ensures we only consume iterUpdates() once (per Zayne's review)
   */
  ...
  /**
   * Safely convert chatId to BigInt for TDLib
   * Throws if ID cannot be safely represented
   */
  private safeChatId(chatId: string): bigint {
    try {
      return BigInt(chatId);
    } catch {
      throw new Error(`[Telegram MTProto] Invalid chat ID: ${chatId}`);
    }
  }

  /**
   * Safely convert messageId to number
   * Throws if ID exceeds safe integer bounds (message IDs are typically smaller)
   */
  private safeMessageId(messageId: string): number {
    const num = Number(messageId);
    if (!Number.isSafeInteger(num)) {
      throw new Error(`[Telegram MTProto] Message ID ${messageId} exceeds safe integer bounds - cannot safely process`);
    }
    return num;
  }
}
